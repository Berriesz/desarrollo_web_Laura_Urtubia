...
const region_formulario = document.getElementById("region");
const comuna_formulario = document.getElementById("comuna");

for(let i = 0; i < region_comuna.regiones.length; i++){
    const region = region_comuna.regiones[i];
    region_formulario.options[i+1] = new Option(region.nombre, region.numero);
}

function cambiaRegion(){
    comuna_formulario.options.length = 0;
    comuna_formulario.options[0] = new Option("Seleccione una comuna", "");

    let seleccionada = null;
    for(let i = 0; i < region_comuna.regiones.length; i++){
        if (region_comuna.regiones[i].numero == region_formulario.value) {
            seleccionada = region_comuna.regiones[i];
            break;
        }
    }

    if(seleccionada){
        for( let j = 0; j < seleccionada.comunas.length; j++){
            let comuna = seleccionada.comunas[j];
            comuna_formulario.options[j+1] = new Option(comuna.nombre, comuna.id);
        }
    }
}
